...
public class GameController : MonoBehaviour
{
    ...
    public static bool a = true; //為了讓新回合指令只做一次
    public static bool b = true; //避免打出後直接犯規
    public static bool c = true; //為了讓結束指令只做一次
    ...
    void New_Turn()
    {
        ...
        if (a && b) //擊打前(新回合開始)a,b皆為true
        {
            ...
            //每個回合結束時的基本設定
            Check_Victory();
            CueBall.transform.localEulerAngles = Zero3;     //重整xyz軸
            hasTouched = false;
            isFoul = false;
            isInHole = false;
            isOut = false;
